...
<br>
Se crean mediante la función .content-box-yellow[**c()**]
.pull-left[
```{r } 
num<-c(1,2,3)
num
...
```{r } 
matrix(11:30, nrow=5, ncol=4)
```


```{r } 
matrix(11:30, nrow=5, ncol=4, byrow=T)
```
...
![](C:/Users/Roxana/curso-r-analisis-datos/img/pep.png)
...
Un dataframe es una lista cuyos componentes pueden ser vectores, matrices o factores. El número de filas y de columnas coinciden, lo que hace que tenga apariencia de una tabla. 

Se puede crear un dataframe con la función .content-box-yellow[**data.frame()**]

```{r } 
data.frame(sitios=1:4, muestreado=c(T,F,F,T))
```
...
`r library(knitr)`
![](C:/Users/Roxana/curso-r-analisis-datos/img/data-types-tidyverse.png){width=50%}
